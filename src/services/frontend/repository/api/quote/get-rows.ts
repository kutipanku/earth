import { PAGE_TYPE } from '@frontend/entity/quote/constants';
import { readRowsData } from '../shared/fetcher';
import { constructOwnSystemRowData } from './normalizer';

import type { Quote, QuoteFilter } from '@frontend/entity/quote/types';
import type { GetQuotes } from './types';

type GetNationalitiesResponse = GetQuotes['response'];

interface Props {
  page?: number;
  rowPerPage?: number;
  filter: QuoteFilter;
}

/**
 * Read rows data to relative module's data source.
 */
const getQuoteRows = async ({ page = 0, rowPerPage = 100, filter }: Props) => {
  const processedFilter = Object.fromEntries(
    Object.entries(filter).filter(
      ([_, value]) => value !== null && value !== undefined
    )
  );

  try {
    const response = await readRowsData<GetNationalitiesResponse>({
      identifier: PAGE_TYPE,
      page,
      rowPerPage,
      filterString: new URLSearchParams(processedFilter).toString(),
    });

    return {
      succsess: response.success,
      message: null,
      data: {
        list: constructOwnSystemRowData(response.data.list),
        total: response.data.total,
      },
    };
  } catch (error) {
    return {
      success: false,
      message: error,
      data: {
        list: [] as Quote[],
        total: 0,
      },
    };
  }
};

export default getQuoteRows;
